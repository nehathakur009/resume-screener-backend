...
const uploadResume = async (req, res) => {
  try {
    ...
    try {
      ...
      console.log('DEBUG: Extracted text from ' + req.file.originalname + ': ' + rawText.substring(0, 500) + (rawText.length > 500 ? '...' : ''));
    } catch (parseErr) {
      ...
      if (parseErr.message.includes('unsupported format') || parseErr.message.includes('invalid')) {
        ...
        console.log('DEBUG: Parse error for ' + req.file.originalname + ': ' + parseErr.message);
        return res.status(422).json({
    ...
    if (!rawText || rawText.trim().length < 20) {
      console.log('DEBUG: Empty or insufficient text extracted from ' + req.file.originalname + '. Length: ' + (rawText ? rawText.length : 'null'));
      return res.status(422).json({
